...
export const getCreateScheduleSolveResponseMock = (overrideResponse: Partial<Extract<ScheduleSolveCreateResponse, object>> = {}): ScheduleSolveCreateResponse => ({id: faker.string.uuid(), status: faker.string.alpha({length: {min: 10, max: 20}}), template_id: faker.string.uuid(), parameters: {population_size: faker.number.int({min: 5, max: 1000}), max_generations: faker.number.int({min: 1, max: 10000}), mutation_rate: faker.number.float({min: 0, max: 1, fractionDigits: 2}), selection_top_n: faker.number.int({min: 1, max: 1000}), elite_size: faker.number.int({min: 0, max: 100})}, ...overrideResponse})
...
export const getCreateScheduleSolveMockHandler = (overrideResponse?: ScheduleSolveCreateResponse | ((info: Parameters<Parameters<typeof http.post>[1]>[0]) => Promise<ScheduleSolveCreateResponse> | ScheduleSolveCreateResponse), options?: RequestHandlerOptions) => {
  return http.post('*/solves/schedule', async (info: Parameters<Parameters<typeof http.post>[1]>[0]) => {
    ...
    : getCreateScheduleSolveResponseMock(),
      { status: 202
      })
  }, options)
}
...
export const getGetScheduleSolveStatusMockHandler = (overrideResponse?: ScheduleSolveStatus | ((info: Parameters<Parameters<typeof http.get>[1]>[0]) => Promise<ScheduleSolveStatus> | ScheduleSolveStatus), options?: RequestHandlerOptions) => {
  return http.get('*/solves/schedule/:solveId', async (info: Parameters<Parameters<typeof http.get>[1]>[0]) => {
    ...
    return HttpResponse.json(overrideResponse !== undefined
    ? (typeof overrideResponse === "function" ? await overrideResponse(info) : overrideResponse)
    : getGetScheduleSolveStatusResponseMock(),
      { status: 200
      })
  }, options)
}

export const getGetScheduleSolveResultMockHandler = (overrideResponse?: ScheduleSolveResult | ((info: Parameters<Parameters<typeof http.get>[1]>[0]) => Promise<ScheduleSolveResult> | ScheduleSolveResult), options?: RequestHandlerOptions) => {
  return http.get('*/solves/schedule/:solveId/result', async (info: Parameters<Parameters<typeof http.get>[1]>[0]) => {


    return HttpResponse.json(overrideResponse !== undefined
    ? (typeof overrideResponse === "function" ? await overrideResponse(info) : overrideResponse)
    : getGetScheduleSolveResultResponseMock(),
      { status: 200
      })
  }, options)
}
