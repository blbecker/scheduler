...
def test_worker_service_get_all(mock_uow):
    # Setup
    worker_model = WorkerModel(id=UUID(int=1), name="John Doe")
    ...
    with patch(
        "scheduler_api.services.worker_service.WorkerRepository", return_value=mock_repo
    ):
...
def test_skill_service_get_all(mock_uow):
    ...
    skill_model = SkillModel(
        id=UUID(int=1), name="Python", description="Python programming"
    )
    ...
    # Patch the repository constructor to return our mock
    with patch(
        "scheduler_api.services.skill_service.SkillRepository", return_value=mock_repo
    ):
...
def test_shift_service_get_all(mock_uow):
    ...
    # Patch the repository constructor to return our mock
    with patch(
        "scheduler_api.services.shift_service.ShiftRepository", return_value=mock_repo
    ):
