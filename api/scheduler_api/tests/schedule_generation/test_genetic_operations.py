...
class TestGeneticOperations:
    ...
    def test_generate_initial_population(self, sample_schedule_layout, mock_random_uniform):
    ...
    def test_breed_population(self, sample_population, mock_random_random):
        ...
        # Check child names
        child_names = [s.name for s in bred_population.schedules if s.name.startswith("Child")]
        assert len(child_names) == 2
        assert "Child_0" in child_names
        assert "Child_1" in child_names
        
        # Check child fitness (average of parents)
        child_fitness = [s.fitness for s in bred_population.schedules if s.name.startswith("Child")]
    ...
    def test_cull_population_no_culling_needed(self, sample_population):
        ...
        culled_population = cull_population(sample_population, target_size=original_size + 5)
    ...
    def test_genetic_pipeline_integration(self, sample_schedule_layout, mock_random_uniform, mock_random_random):
