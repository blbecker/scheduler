...
class TestScheduleLayoutService:
    ...
    def test_update_layout_found(self, service, mock_repository, sample_update_dto):
        ...
        with patch("scheduler_api.db.models.ScheduleLayoutUpdate") as mock_db_update:
            ...
            mock_repository.update.assert_called_once_with(
                layout_id, mock_db_update_instance
            )
            assert isinstance(result, ScheduleLayoutResponse)
    ...
    def test_update_layout_not_found(self, service, mock_repository, sample_update_dto):
        ...
        with patch(
            "scheduler_api.services.schedule_layout_service.ScheduleLayoutUpdate"
        ):
    ...
    def test_generate_schedule_success(self, service, mock_repository):
        ...
        with patch(
            "scheduler_api.services.schedule_layout_service.generate_schedule_from_layout"
        ) as mock_task:
    ...
    def test_generate_schedule_layout_not_found(self, service, mock_repository):
        ...
        # Execute & Verify
        with pytest.raises(
            ValueError, match=f"Schedule layout with ID {layout_id} not found"
        ):
