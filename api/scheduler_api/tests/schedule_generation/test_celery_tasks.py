...
class TestCeleryTasks:
    ...
    def test_population_generate_task(
        ...
        with patch(
            "scheduler_api.tasks.ga_tasks.generate_initial_population"
        ) as mock_gen_pop:
            with patch(
                "scheduler_api.tasks.ga_tasks.schedule_layout_from_dto"
            ) as mock_from_dto:
                with patch(
                    "scheduler_api.tasks.ga_tasks.population_to_dto"
                ) as mock_to_dto:
                    # Setup mocks
                    ...
                    # Execute task
                    result = population_generate(
                        sample_schedule_layout_dto.model_dump()
                    )
    ...
    def test_score_population_task(
        ...
        with patch("scheduler_api.tasks.ga_tasks.score_population") as mock_score_pop:
            with patch(
                "scheduler_api.tasks.ga_tasks.population_from_dto"
            ) as mock_from_dto:
                with patch(
                    "scheduler_api.tasks.ga_tasks.population_to_dto"
                ) as mock_to_dto:
            ...
            with patch("scheduler_api.tasks.ga_tasks.breed_population") as mock_breed:
                with patch(
                    ...
                    with patch(
                        ...
                        mock_population_dto = MagicMock()
                        mock_population_dto.model_dump.return_value = {
                            "test": "mutated_breed"
                        }
                        mock_to_dto.return_value = mock_population_dto
                        ...
                        mock_mutate.assert_called_once_with(
                            mock_population, mutation_rate=0.1
                        )
                        mock_breed.assert_called_once_with(
                            mock_mutated_population, crossover_rate=0.8
                        )
    ...
    def test_select_best_task(
        ...
        with patch("scheduler_api.tasks.ga_tasks.select_best_schedule") as mock_select:
            with patch(
                ...
                with patch(
                    "scheduler_api.tasks.ga_tasks.schedule_to_dto"
                ) as mock_to_dto:
                    # Setup mocks
                    mock_population = MagicMock()
                    mock_from_dto.return_value = mock_population
    ...
    def test_task_error_handling(self, sample_schedule_layout_dto):
        ...
        with patch(
            "scheduler_api.tasks.ga_tasks.schedule_layout_from_dto",
            side_effect=ValueError("Invalid DTO"),
        ):
        ...
        with patch(
            "scheduler_api.tasks.ga_tasks.population_from_dto",
            return_value=MagicMock(schedules=[]),
        ):
            with patch(
                "scheduler_api.tasks.ga_tasks.select_best_schedule",
                side_effect=ValueError("No schedules"),
            ):
...
class TestTaskIntegration:
    ...
    def test_task_chain_structure(self, mock_celery_chain, sample_schedule_layout_dto):
        ...
        assert (
            task_names == expected_order
        ), f"Tasks should be chained in order: {expected_order}"
    ...
    def test_task_data_flow(self, sample_schedule_layout_dto):
        ...
        mock_results = [
            {"id": "pop-1", "schedules": []},  # population_generate result
            {
                ...
                "schedules": [],
                "scored": True,
            },  # score_population_task result
            {
                "id": "pop-1",
                "schedules": [],
                "mutated": True,
            },  # mutate_and_breed result
        ...
        for i, result in enumerate(mock_results):
            ...
            mock_task.apply = MagicMock(
                return_value=MagicMock(get=MagicMock(return_value=result))
            )
        ...
        with patch("scheduler_api.tasks.ga_tasks.chain") as mock_chain:
            ...
            mock_chain_instance.return_value.get = MagicMock(
                return_value=mock_results[-1]
            )

            # Execute entrypoint
            result = generate_schedule_from_layout(
                sample_schedule_layout_dto.model_dump()
            )
