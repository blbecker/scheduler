"""Skills match scorer for evaluating schedules."""

from dataclasses import dataclass
from uuid import UUID
from scheduler_api.engine.interfaces import Scorable
from scheduler_api.domain.schedule import Schedule
from ..context import ScheduleSolveContext


@dataclass
class SkillsMatchScorer(Scorable[Schedule, ScheduleSolveContext]):
    """Scorer that evaluates skill matches between workers and shifts."""

    name: str = "skills_match_scorer"

    def score(self, schedule: Schedule, context: ScheduleSolveContext) -> float:
        """Calculate skill match score for schedule."""
        if not schedule.shift_assignments:
            return 0.0
        ...
        for assignment in schedule.shift_assignments:
            ...
            assignments_by_shift[assignment.shift_id].append(assignment.worker_id)

        for shift_id, assigned_workers in assignments_by_shift.items():
